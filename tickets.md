...
| CHORE | Add CI workflow for lint, type, and smoke tests | Add GitHub Actions pipeline for `ruff`, `ty`, `pytest`, and workflow-compliance gates on push/PR | IN_PROGRESS |
...
| CHORE | Configure protected branch required status checks | Require `quality` and `workflow_compliance` checks before merge in GitHub settings | TODO |
